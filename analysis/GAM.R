...
df <- read.csv('raw_cohort_bp.csv', stringsAsFactors = TRUE)

df[c('ethnicity')] = apply(df[c('ethnicity')], 1, mergeEth)
df$heparin_iv[is.na(df$heparin_iv)] <- 0
df$antihypertensive_iv[is.na(df$antihypertensive_iv)] <- 0
...
df['antihypertensive'] = as.numeric(unlist(df[c('antihypertensive_iv')])) + as.numeric(unlist(df['antihypertensive_non_iv']))

df[c('afib', 'hyperlipidemia', 'diabetes', 'hypertension',
    'cor_art_d', 'peri_vasc_d', 'car_art_stent','smoking',
    'tia', 'heparin_iv', 'antihypertensive_iv', 'antiplatelets',
    'anticoag', 'antihypertensive_non_iv')] = apply(df[c('afib', 'hyperlipidemia', 'diabetes', 'hypertension',
                   'cor_art_d', 'peri_vasc_d', 'car_art_stent','smoking',
                   'tia', 'heparin_iv', 'antihypertensive_iv', 'antiplatelets',
                   'anticoag', 'antihypertensive_non_iv')], c(1,2), cap)


categoricals <- c('ethnicity', 'gender', 'hospital_expire_flag',
                  'afib', 'hyperlipidemia', 'diabetes', 'hypertension',
                  'cor_art_d', 'peri_vasc_d', 'car_art_stent','smoking',
                  'tia', 'heparin_iv', 'antihypertensive', 'antiplatelets',
                  'anticoag')
continuous <- c('age','avg_mbp_ni')
